...
public class MonCardData {
	...
	public MonCardData(byte[] rom, int startIndex){
		...
		type = rom[c++];
		gfx = new Word(rom,c); c+=2;//I want every var to be one line, so words increment on the same line
		...
		weakness = rom[c++];
		resistance = rom[c++];
		kind = new Word(rom,c); c+=2;
		pokedexNumber = rom[c++];
		unknownByte1 = rom[c++];
		level = rom[c++];
		length = new Word(rom,c); c+=2;
		weight = new Word(rom,c); c+=2;
